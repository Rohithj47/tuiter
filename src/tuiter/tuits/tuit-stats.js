...
const TuitStats = (
    ...
    const toggleLike = () => {
        if (liked){
            ...
            dipatch(updateTuitThunk({ ...tuit, likes: tuit.likes - 1, liked: false}))
        }else{
            setLiked(true);
            setLikes((setLikes) => setLikes += 1)
    ...
    const toggleDislike = () => {
        if (disliked){
            ...
            dipatch(updateTuitThunk({ ...tuit, dislikes: tuit.dislikes - 1, disliked: false}))
        }else{
            setdisliked(true);
            setdislikes((dislikes) => dislikes += 1)
            dipatch(updateTuitThunk({ ...tuit, dislikes: tuit.dislikes + 1, disliked: true}))
        }
    }
    ...
    return (
        ...
        <div className="col">
            <button onClick={() => toggleDislike()} style={{ background: 'none', border: 'none' }} >
                {
                    disliked?  <FontAwesomeIcon className="mx-2" icon={solidDislike} />
